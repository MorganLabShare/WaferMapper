...
if AlignTargetPointListGuiGlobalStruct.IsShowOnlyCurrentImage
    if ~isempty(AlignTargetPointListGuiGlobalStruct.CurrentImage)
        ...
        ColorCombinedImage(:,:,3) = ColorCombinedImage(:,:,1);
    else
        ColorCombinedImage = [];
    end
else
...
half_h = AlignedTargetList.WaferArray(WaferNameIndex).SectionArray(SectionIndex).half_h;
 ...
 NewImageFull = imread(ImageFileNameStr);
[NewImageFullSizeWidth NewImageFullSizeHeight] = size(NewImageFull);
...
%RS Code added to prevent indexing error when the
%GenerateListOfAlignedTargetPoints function generates r and c offsets which
%exceed the bondary of the ??

       if New_r-(half_h*3+1) <=0 
           if New_c-(half_h*3+1)<= 0
               AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
                {[0 New_r+(half_h*3+1)], [0 New_c+(half_w*3+1)]});
                rdiff = abs((New_r-(half_h*3+1)));
                cdiff = abs((New_c-(half_w*3+1)));
           else
               AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
                {[0 New_r+(half_h*3+1)], [New_c-(half_w*3+1) New_c +(half_w*3+1)]});
                rdiff = abs((New_r-(half_h*3+1)));
           end
       end     
           
       if New_r-(half_h*3+1) >0 & New_c-(half_h*3+1)<= 0
           AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
                {[New_r-(half_h*3+1) New_r+(half_h*3+1)], [0 New_c+(half_w*3+1)]});
                cdiff = abs((New_c-(half_w*3+1)));
           
         
      
            
       if   New_r+(half_h*3+1) >= NewImageFullSizeWidth
           if New_c+(half_w*3+1)>= NewImageFullSizeWidth 
               AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
                {[New_r-(half_h*3+1) NewImageFullSizeWidth], [New_c-(half_w*3+1) NewImageFullSizeWidth]});
                rdiff = abs(New_r+(half_h*3+1) - NewImageFullSizeWidth);
                cdiff = abs(New_c+(half_w*3+1) - NewImageFullSizeWidth);
           else
               AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
                {[New_r-(half_h*3+1) NewImageFullSizeWidth], [New_c-(half_w*3+1) New_c+(half_w*3+1)]});
                rdiff = abs(New_r+(half_h*3+1) - NewImageFullSizeWidth);
           end    
       end
     
       if New_r-(half_h*3+1) < NewImageFullSizeWidth & New_c+(half_w*3+1)>= NewImageFullSizeWidth
           AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
                {[New_r-(half_h*3+1) New_r+(half_h*3+1)], [ New_c-(half_w*3+1) NewImageFullSizeWidth]});
                cdiff = abs(New_c+(half_w*3+1)- NewImageFullSizeWidth);
       end         
                         
       if 0 <= New_r-(half_h*3+1) & New_r+(half_h*3+1) <= NewImageFullSizeHeight & 0 <= New_c-(half_w*3+1) & New_c+(half_w*3+1) <= NewImageFullSizeWidth         
             AlignTargetPointListGuiGlobalStruct.NewImage_x3LargerROI_shifted = imread(ImageFileNameStr, 'PixelRegion',...
            {[New_r-(half_h*3+1) New_r+(half_h*3+1)], [New_c-(half_w*3+1) New_c+(half_w*3+1)]});
             rdiff= 0;
             cdiff=0;
