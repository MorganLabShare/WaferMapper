...
%Determine downsample factor
 OverviewImageFileNameStr = sprintf('%s\\SectionOverview_1.tif',GuiGlobalsStruct.SectionOverviewsDirectory);
 Iinf = imfinfo(OverviewImageFileNameStr);
  
TempWidth = Iinf.Width;
...
for i = 1:MaxLabelNum
        Label = num2str(i);
       OverviewAlignedDataFileNameStr{i} = sprintf('%s\\SectionOverviewAligned_%s.mat',GuiGlobalsStruct.SectionOverviewsAlignedWithTemplateDirectory,Label);
end

parfor i = 1:MaxLabelNum
    %disp(sprintf('Aligning section %d of %d', i,MaxLabelNum));
    dirTif = dir([GuiGlobalsStruct.SectionOverviewsAlignedWithTemplateDirectory '\*.tif']);
    disp(sprintf('%d of %d sections completed',length(dirTif),MaxLabelNum));
...
%     
    ...
    AlignmentParameters.AngleOffsetInDegrees = AngleOffsetInDegrees;
   	AP(i).AlignmentParameters = AlignmentParameters;
    %save(OverviewAlignedDataFileNameStr{i}, 'AlignmentParameters');
   % parsav(OverviewAlignedDataFileNameStr{i}, AlignmentParameters);
