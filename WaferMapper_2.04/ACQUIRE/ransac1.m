function [f inlierIdx] = ransac1( x,y,ransacCoef,funcFindF,funcDist,funcDet )
    ...
    for p = 1:iterNum
        ...
        %check to make sure we didn't end up with the same destination point
        if isempty(find(sum(diff(y(:,sampleIdx),2))==0))
